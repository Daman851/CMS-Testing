...
namespace CMS_Testing.Tests
{
    ...
    class News_Test : Basetest
    {

        /* taking the reference for screenshot for the baseclass*/
        ExtentReports Newsscreenshot = Basetest.getInstance();       // Declaring Getinstance as screenshot method //
        [Test, Category("Automation Testing")]       // Grouping the test method //
        [TestCaseSource(typeof(Basetest), "browsertorunwith")]
        public void Tc8(string browsername)             // Creating the method //
        {
            
            parallelbrowser(browsername);           //  calling the function from the basetest for the parallel browser test //
            var lp = new News(driver);         // Creating an instance of base page(News) //
            Thread.Sleep(2000);
            lp.verifyingnews();                //Calling the base page methods(Verifying News)//
            Thread.Sleep(2000);
        }

        [Test, Category("Automation Testing")]      // Grouping the test method //
        [TestCaseSource(typeof(Basetest), "browsertorunwith")]
        public void Tc23(string browsername)               // Creating the method //
        {

            parallelbrowser(browsername);        //  calling the function from the basetest for the parallel browser test //
            var lp = new News(driver);             // Creating an instance of base page(News) //
            Thread.Sleep(2000);
            lp.verifyreadmorearticle1();      //Calling the base page methods(Verifyingreadmorearticle1)//
            Thread.Sleep(2000);
        }
        ...
        public void Tc56(string browsername)             // Creating the method //
        {

            parallelbrowser(browsername);        //  calling the function from the basetest for the parallel browser test //
            var lp = new News(driver);                 // Creating an instance of base page(News) //
            Thread.Sleep(2000);
            lp.verifyingreadmorearticle2();             //Calling the base page methods(Verifyingreadmorearticle2)//
            Thread.Sleep(2000); 
        }
        ...
        public void screenshotteardown()        //Creating a method for Extent reports //
        {
            ...
            test = extent.CreateTest("<h3>News</h3>").Info("<h3>Test Started in </h3>" + browsername + " " + "and" + " " + methodname);
