    ...
    class LocationTest : Basetest
{
    ...
    ExtentReports Locationscreenshot = Basetest.getInstance();       // Declaring Getinstance as screenshot method //
    ...
    public void Verifying_Locations(string browsername)          // Creating the method //
        {

        parallelbrowser(browsername);      //  calling the function from the basetest for the parallel browser test //
         var lp = new Location(driver);    // Creating an instance of base page(Location)
            lp.verifyinglocations();          //Calling the base page methods(Verifying_locations)//
            Thread.Sleep(2000);
    }
        // Everytime report should be cleared , or refreshed //
        [TearDown]

    public void screenshotteardown()
    {
        ExtentTest test;
            ...
            test = extent.CreateTest("<h3>Locations</h3>").Info("<h3>Test Started in </h3>" + browsername +" " + "and" + " "+ methodname) ;
