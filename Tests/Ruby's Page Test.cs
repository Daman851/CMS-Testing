...
namespace CMS_Testing.Tests
{
    ...
    class Ruby_s_Page_Test : Basetest
    {

        /* taking the reference for screenshot for the baseclass*/
        ExtentReports Ruby_sPagescreenshot = Basetest.getInstance();       // Declaring Getinstance as screenshot method //
        ...
        public void Tc11(string browsername)       // Creating the method //
        {
            
            parallelbrowser(browsername);      //  calling the function from the basetest for the parallel browser test //
            var lp = new Class1(driver);         // Creating an instance of base page(Ruby's Page)
            lp.verifyingrubypage();        //Calling the base page methods(Verifying Ruby's Page)//
            Thread.Sleep(2000);
        }
        [TearDown]
